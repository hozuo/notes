...
- 值:通常是id选择器(或者是一个 HTMLElement 实例),如果管理类选择器只能对第一个匹配的标签生效
- el不能直接管理视图html或者body

##### [data](https://cn.vuejs.org/v2/api/#data)

- Vue 实例的数据对象，是响应式数据(数据驱动视图),数据改变时视图会自动发生变化
- 可以通过 `vm.$data` 访问原始数据对象
- Vue 实例也代理了 data 对象上所有的属性，因此访问 `vm.message` 等价于访问 `vm.$data.message`
...
>
> vue可以根据这些key定位标签,渲染的效率会更高,(前提是必须唯一)
...
```html
 <div id="app">
        <div v-bind:class="{active: isActive,left: isLeft}">
            hello
        </div>
...
在数组中存入变量的名,class是变量的值,也可以在数组中存对象,此时遵循对象语法
...
```html
 <div id="app">
        ...
        <hr>
        <!-- v-model其实是语法糖,它是下面这种写法的简写 $event指事件,target此时指表单标签-->
...
- ##### 绑定多个复选框
  ...
  此种方式需要input标签**提供value属性**
   ...
   v-model绑定的是**数组**,选中标签的value值会存入数组,此时数组为["Jack","John","Mike"]
...
<script src="./vue.js"></script>
<script>
    // 1. 定义全局过滤器 也可以使用function(value){}定义函数,value指的是调用过滤器的数据
...
>
> 2. 过滤器是可以传参数的,比如{{msg | upper1(a,b)}}
>
>    此时函数第一个参数是值,后面依次是自定义参数,调用时不需要显示的写数据值
>
>    upper1:(v,a,b)=>{}
...
<script>
    var vm = new Vue({
        ...
        directives: {
            // 2. directives:{'指令名称':{ inserted: (DOM对象) => { DOM操作 } }}
            focus: {
                // 指令的定义
...
2. 设置HTML中的内容,to的值可以是路径,变量,对象,对象的name和path字段对应路由的name和path
...
tag属性指定编译的标签类型,默认是a标签

```
 <router-link to="/instock" tag="button">入库流水</router-link>
```
...
2. 通过 <router-link> 传参，在路径上传入具体的值
   ...
   <router-link to="/users/:id">用户管理2</router-link>
   ```

3. 在组件内部可以使用，**this.$route** 获取当前路由配置对象,**$route.params.id**获取动态参数
...
![image-20200425001448748](C:\Users\Ericson\AppData\Roaming\Typora\typora-user-images\image-20200425001448748.png)
...
<body>
  ...
  <div id="app" v-cloak>
    ...
    <router-link to="/point" tag="li"><a>热点</a></router-link>
  ...
  <script>
    var home = {
      template: '<div>主页主页主页主页主页主页主页主页</div>'
    }

    var point = {
      template: '<div>热点热点热点热点热点热点热点热点</div>'
    }
    ...
    var musicSub = {
      template: '<div>musicSub {{$route.params.id}}</div>'
    }
...
- .babelrc  babel配置参数,用于把es6转换为es3
- .editorconfig 编辑器配置文件,指定了代码格式
- .gitignore git忽略文件,拒绝git提交
- index.html 项目的默认首页
- package-lock.json 锁定项目安装时相关依赖的版本和下载链接,避免克隆后再次安装时版本不一致
- package.json 项目各种包的版本 其中scripts可以简化指令,类似于宏定义
- README.md 项目说明
...
`AppHeader.vue` (使用sca快捷代码段,把所有标签放入template中)
...
<template>
    <div>
    <!-- 3 使用AppHeader.vue组件 驼峰命名可以自动转换为短线调用 -->
...
1. router.js 导入vue,vue_router,Vue.use(VueRouter);
2. router.js 导入要渲染的组件
3. router.js 配置路由routes[{}]
4. router.js 实例化路由对象new VueRouter({routes})
5. router.js 导出router对象
6. main.js 导入路由对象
7. main.js vue对象挂载路由
...
<img src="C:\Users\Ericson\AppData\Roaming\Typora\typora-user-images\image-20200425210815098.png" alt="image-20200425210815098" style="zoom: 67%;" />

<img src="C:\Users\Ericson\AppData\Roaming\Typora\typora-user-images\image-20200425210619151.png" alt="image-20200425210619151" style="zoom: 67%;" />
...
// 导入axios
import axios from 'axios';
// 配置所有Vue的实例都具有$http这个成员,$http可以换成其他值例如Vue.prototype.axios = axios;
...
![image-20200426212844417](C:\Users\Ericson\AppData\Roaming\Typora\typora-user-images\image-20200426212844417.png)
...
在package.json中的"scripts": {}下定义

`"lintfix": "eslint --ext .js,.vue src --fix"`

运行 npm run lintfix可以自动修正js的格式错误(一般用不到,格式错误在ide里就修复了)

取消规则,build/webpack.base.conf.js 注释掉rules

![image-20200426215701157](C:\Users\Ericson\AppData\Roaming\Typora\typora-user-images\image-20200426215701157.png)
...
3. git add .
4. git commit -m 注释
5. 在代码托管平台（ git hub）新建远程仓库
6. git config --global user.name "ericson"
7. git config --global user.email "ghzfree@163.com"
8. git remote add origin https://github.com/hozuo/mallManager.git
9. git push -u origin master
10. 以后的提交直接使用git push
...
Switched to a new branch 'dev-login'
` git branch`

*dev-login
master

![image-20200427002830331](C:\Users\Ericson\AppData\Roaming\Typora\typora-user-images\image-20200427002830331.png)
...
git push --set-upstream origin dev-power //提交到分支
```
...
这种方法会把参数多一层封装使得springmvc无法解析,下面的方法借助URLSearchParams去掉了封装
...
在获得异步请求数据的位置前加await,并接收值,在这个值最近的方法前加async进行改造,不改变真实的代码逻辑(仍然是异步操作)
...
要做的是把OPTIONS请求处理后直接返回,其他请求处理跨域后放行
...
单元格中的内容不是文本,就要在外层包裹template组件,该组件需要slot-scope传值,该值是形参代表最外层遍历的数据源(此时是list数组),数组有row属性代表当前的对象
...
![image-20200430212751650](C:\Users\Ericson\AppData\Roaming\Typora\typora-user-images\image-20200430212751650.png)